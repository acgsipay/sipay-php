...
class PreauthorizationsCardsAuth extends Action
{
    ...
    public function call($amount, $ticket, $templateid, $dstpage, array $extra = array())
    {
        ...
        $input = array(
            ...
            'resource'              => 'iframe/v1/redirects/preauthorizations/cards'
        );
        ...
        $this->log->info('sipay.actions.redirect.preauthorizations_cards_auth', 'api.request', 'I-00001', 'Send Request', $params);

        $this->request->json($params)->call();

        if($this->request->error == false) {
            $this->log->info('sipay.actions.redirect.preauthorizations_cards_auth', 'api.response', 'I-00001', 'Request OK', $this->request->json);
            return $this->request->get('redirects.url');
        }

        $this->log->error('sipay.actions.redirect.preauthorizations_cards_auth', 'api.response', 'E-00001', 'Request KO', $this->request->error);
